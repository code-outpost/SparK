/* =========================================================================
   SparK · 数字能源测试效率工具箱  app.js  (v1.1)
   全部逻辑本地运行，无网络依赖。
   ========================================================================= */
(function(){
'use strict';
console.log('SparK app.js v20260829-3 loaded');
...
function jdAnalyze(){
  ...
  if(!sel.length){toast('请先在左侧岗位前勾选至少一个（可多选对比）');return;}
  var N=sel.length;
...
var PF_MAX_SEG=8;   // 一次调频自定义公式最大转折段数（可用变量 k1~k8 / u1~u8）
function addPfRow(){var b=document.getElementById('pf-body'),n=b.rows.length+1;if(n>PF_MAX_SEG){toast('最多 '+PF_MAX_SEG+' 段');return;}
...
/* ---------- 安全表达式求值（递归下降，替代 new Function） ----------
   支持：数字 / 变量 / + - * / % ^ / 比较 < > <= >= == != / 逻辑 && || /
        三元 ?: / 括号 / 函数 abs min max sqrt pow floor ceil round
   不再使用 new Function，避免任意代码执行。 */
function tokenizeExpr(s){
  ...
  while(i<n){
    ...
    var m=false,q;
    for(q=0;q<TWO.length;q++){ if(s.substr(i,2)===TWO[q]){out.push({t:'op',v:TWO[q]});i+=2;m=true;break;} }
    if(m)continue;
    for(q=0;q<ONE.length;q++){ if(s[i]===ONE[q]){out.push({t:'op',v:ONE[q]});i++;m=true;break;} }
    if(m)continue;
    throw new Error('unexpected char "'+c+'"');
  }
  return out;
}
var EXPR_FN={abs:Math.abs,sqrt:Math.sqrt,floor:Math.floor,ceil:Math.ceil,round:Math.round,
             min:Math.min,max:Math.max,pow:Math.pow};
function evalFormula(formula,vars){
  try{
    var toks=tokenizeExpr(String(formula||''));
    var pos=0,EOF={t:'eof'};
    function peek(){return toks[pos]||EOF;}
    function next(){return toks[pos++]||EOF;}
    function isOp(v){var t=peek();return t.t==='op'&&t.v===v;}
    function eat(v){if(isOp(v)){next();return true;}return false;}
    function parseExpr(){return parseTern();}
    function parseTern(){
      var c=parseOr();
      if(eat('?')){var a=parseExpr();if(!eat(':'))throw new Error('expect :');
        var b=parseExpr();return c?a:b;}
      return c;
    }
    function parseOr(){
      var l=parseAnd();
      while(isOp('||')){next();var r=parseAnd();l=(l||r)?1:0;}
      return l;
    }
    function parseAnd(){
      var l=parseEq();
      while(isOp('&&')){next();var r=parseEq();l=(l&&r)?1:0;}
      return l;
    }
    function parseEq(){
      var l=parseRel();
      for(;;){
        if(isOp('==')){next();l=(l===parseRel())?1:0;}
        else if(isOp('!=')){next();l=(l!==parseRel())?1:0;}
        else return l;
      }
    }
    function parseRel(){
      var l=parseAdd();
      for(;;){
        if(isOp('<')){next();l=(l<parseAdd())?1:0;}
        else if(isOp('>')){next();l=(l>parseAdd())?1:0;}
        else if(isOp('<=')){next();l=(l<=parseAdd())?1:0;}
        else if(isOp('>=')){next();l=(l>=parseAdd())?1:0;}
        else return l;
      }
    ...
    function parseAdd(){
      var l=parseMul();
      for(;;){
        if(isOp('+')){next();l=l+parseMul();}
        else if(isOp('-')){next();l=l-parseMul();}
        else return l;
      }
    }
    function parseMul(){
      ...
      for(;;){
        if(isOp('*')){next();l=l*parseMul();}
        else if(isOp('/')){next();var d=parseMul();l=d===0?0:l/d;}
        else if(isOp('%')){next();var m2=parseMul();l=m2===0?0:l%m2;}
        else return l;
      }
    }
    // ^ 右结合
    function parsePow(){
      var b=parseUnary();
      if(isOp('^')){next();return Math.pow(b,parsePow());}
      return b;
    }
    function parseUnary(){
    ...
    function parsePrimary(){
      ...
      if(t.t==='id'){
        ...
        if(Object.prototype.hasOwnProperty.call(vars||{},t.v))return vars[t.v];
        if(t.v==='true')return 1;
        if(t.v==='false')return 0;
        throw new Error('unknown var '+t.v);
      }
      if(t.t==='op'&&t.v==='('){var e=parseExpr();if(!eat(')'))throw new Error('expect )');return e;}
      throw new Error('unexpected token');
    }
    var r=parseExpr();
    if(pos<toks.length)throw new Error('trailing tokens');
    return (r===null||r===undefined||typeof r!=='number'||isNaN(r)||!isFinite(r))?0:r;
  }catch(e){return 0;}
}
function calcPf(){
  ...
  var formula=document.getElementById('pf-formula').value.trim();if(!formula){toast('请先选择或输入公式');return;}
  var vars={f:f,fn:fn,fd:fd,Pn:Pn};
  for(var i=1;i<=PF_MAX_SEG;i++){
...
function initHero(){
  ...
  // P3：离开视口时暂停动画，避免切走页面后仍常驻 rAF 空转
  ...
  function start(){ if(running)return; running=true; if(rafId!=null)cancelAnimationFrame(rafId); rafId=requestAnimationFrame(step); }
  function stop(){ running=false; if(rafId!=null){cancelAnimationFrame(rafId);rafId=null;} }
  try{resize();seed();}catch(e){}
  // 仅当首页 hero 在视口内才跑动画
  var heroHost=cv.parentNode||cv;
  if(typeof IntersectionObserver!=='undefined'){
    try{
      new IntersectionObserver(function(es){
        for(var i=0;i<es.length;i++){ if(es[i].isIntersecting)start(); else stop(); }
      },{threshold:0}).observe(heroHost);
    }catch(e){ start(); }
  } else { start(); }
...
/* Canvas 图表引擎                                                      */
/* ====================================================================== */
function CanvasChart(ctx,canvas){this.ctx=ctx;this.canvas=canvas;
  ...
  this.resize();
  /* 不再各自挂 window resize；统一由 resizeCharts 防抖调度（见下方 P1 重构） */}
...
/* 图表 resize 总控
   P1 重构：
   1) 用统一登记表 + 单个防抖监听，替代「每个图表实例各自挂 window resize」；
   2) 记录最近一次绘图调用，resize 后按原参数重画——修复了旧实现里 _drawFn
      闭包捕获旧 W/H、导致缩放后仍按老尺寸绘制的 bug；
   3) 隐藏（display:none 的 section）的图表跳过，省掉无效重绘。            */
var CHART_IDS=['pv','pf','qu','sa','pc','soc','sym','radar'];
var _chartReg=[];
function registerChart(ch){ if(ch&&ch.canvas&&_chartReg.indexOf(ch)<0)_chartReg.push(ch); }
/* 包装各绘图方法：缓存调用参数，供 resize 后按新尺寸重绘 */
['bar','hBar','quLine','pfCurve','radar','line'].forEach(function(m){
  var orig=CanvasChart.prototype[m];
  if(typeof orig!=='function')return;
  CanvasChart.prototype[m]=function(){
    var self=this,args=arguments;
    var r=orig.apply(this,args);
    // 注意：重绘必须重新执行 orig（内部会重新读取 canvas.width/height），
    // 而不是复用 _drawFn（它闭包了旧尺寸）。
    self._redraw=function(){return orig.apply(self,args);};
    registerChart(self);
    return r;
  };
});
function resizeCharts(){
  for(var i=0;i<_chartReg.length;i++){
    var ch=_chartReg[i],cv=ch.canvas;
    if(!cv||!cv.isConnected)continue;
    if(cv.offsetParent===null)continue;          // 隐藏的 section，跳过
    try{ ch.resize(); if(ch._redraw)ch._redraw(); }catch(e){}
  }
}
if(typeof window!=='undefined'){
  var _chartRT;
  window.addEventListener('resize',function(){clearTimeout(_chartRT);_chartRT=setTimeout(resizeCharts,140);});
}
